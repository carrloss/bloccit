class SponsoredPostsController < ApplicationController
  ...
  def show
    @sponsoredp = SponsoredPost.find(params[:id])
  end

  def new
    @topic = Topic.find(params[:topic_id])
    @sponsoredp = SponsoredPost.new
  end

  def create
    @sponsoredpost = SponsoredPost.new
    @sponsoredpost.title = params[:sponsored_post][:title] #Why do i call sponsored post with the lowbar here?
  ...
  def update
    ...
    if @sponsoredpost.save
      flash[:notice] = "Sponsored post was udated successfully."
